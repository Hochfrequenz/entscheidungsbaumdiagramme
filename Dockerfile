...
# NOTE: static/ebd and static/companystylesheet are git submodules. The build context
# must have them checked out or the site builds without its diagrams and styles — CI
# handles this with `submodules: recursive`; locally run `git submodule update --init`.
COPY . .
