...
`ghcr.io/hochfrequenz/entscheidungsbaumdiagramme`; a `-rc` tag is a staging release, a plain version tag is production.
The workflow prints the image digest to pin in the deployment repo.

```sh
$ git tag v1.2.3 && git push origin v1.2.3      # release
$ docker build -t ebd .                       # build locally (needs submodules)
$ docker run --rm -p 8080:8080 \
    -e APP_AUTH0_CLIENT_ID=<client-id> ebd    # run locally
```
...
| `docker/nginx.conf`            | serving rules carried over from `staticwebapp.config.json` |
